...
<!-- SCHEMA -->
<!-- Default uses XSLT 1 -->
    <xsl:template match="iso:schema[not(@queryBinding) or @queryBinding='xslt'       or @queryBinding='xslt1' or @queryBinding='XSLT' or @queryBinding='XSLT1'      or @queryBinding='xpath']">
...
<!-- Using XSLT 2 -->
    <xsl:template match="iso:schema[@queryBinding='xslt2' or @queryBinding ='xpath2']" priority="10">
        <axsl:stylesheet xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:saxon="http://saxon.sf.net/">
            <xsl:apply-templates select="iso:ns"/>
	    <!-- Handle the namespaces before the version attribute: reported to help SAXON -->
...
<!-- Uses unknown query language binding -->
   ...
   for ZIP archives.
	-->
        ...
        <xsl:comment>XSD TYPES FOR XSLT2</xsl:comment>
        <xsl:text>
</xsl:text>
...
<!-- ============================================================== -->
    ...
    <xsl:template match="iso:active">
        ...
        <xsl:if test="not(../../iso:pattern[@id = current()/@pattern])                 and not(../../iso:include)">
            <xsl:message>
                <xsl:call-template name="outputLocalizedMessage">
    ...
    <xsl:template match="iso:assert">
        ...
        <xsl:comment>ASSERT <xsl:value-of select="@role"/>
        </xsl:comment>
        <xsl:text>
</xsl:text>
        <axsl:choose>
    ...
    <xsl:template match="iso:report">
        ...
        <xsl:comment>REPORT <xsl:value-of select="@role"/>
        </xsl:comment>
        <xsl:text>
</xsl:text>
        <axsl:if test="{@test}">
  ...
  <!-- XSL FUNCTION -->
   ...
   <!-- This is only a fallback. Include really needs to have been done before this as a separate pass.-->
    <xsl:template match="iso:include[not(normalize-space(@href))]" priority="1">
   ...
   <!-- This is to handle the particular case of including patterns -->
    ...
    <xsl:template match="iso:pattern" mode="do-all-patterns">
        <xsl:if test="($phase = '#ALL')   or (../iso:phase[@id= $phase]/iso:active[@pattern= current()/@id])">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <axsl:for-each select="$thePath">
                        <axsl:choose>
                            <axsl:when test="starts-with( ., 'http:') or starts-with(., 'file:' )          or starts-with(., '/')"><!-- try as absolute path -->
    ...
    <xsl:template match="iso:pattern[not(@abstract='true')]">
        <xsl:if test="($phase = '#ALL')             or (../iso:phase[@id= $phase]/iso:active[@pattern= current()/@id])">
            ...
            <xsl:comment>PATTERN <xsl:value-of select="@id"/>
    ...
    <xsl:template match="iso:rule[not(@abstract='true')] ">
        ...
        <xsl:comment>RULE <xsl:value-of select="@id"/>
        </xsl:comment>
        <xsl:text>
</xsl:text>
...
<!-- ============================================================== -->
  ...
  <!-- This works with all namespaces -->
        <xsl:if test="not(string-length(normalize-space($start)) = 0)     and not(//iso:diagnostic[@id = $start])   and not(//schold:diagnostic[@id = $start])    and not(//diagnostic[@id = $start])">
            <xsl:message>
                <xsl:call-template name="outputLocalizedMessage">
        ...
        <xsl:if test="string-length(normalize-space($start)) &gt; 0">
            <xsl:text> </xsl:text>
            <xsl:apply-templates select="//iso:diagnostic[@id = $start ]          | //schold:diagnostic[@id = $start ]              | //diagnostic[@id= $start ]"/>
        </xsl:if>
        <xsl:if test="not($end='')">
  ...
  <!-- This works with all namespaces -->
        <xsl:if test="not(string-length(normalize-space($start)) = 0)     and not(//iso:property[@id = $start])">
            <xsl:message>
                <xsl:call-template name="outputLocalizedMessage">
                    <xsl:with-param name="number">36a</xsl:with-param>
                </xsl:call-template>
        ...
        <xsl:if test="string-length(normalize-space($start)) &gt; 0">
            <xsl:text> </xsl:text>
            <xsl:apply-templates select="//iso:property[@id = $start ] "/>
        </xsl:if>
        <xsl:if test="not($end='')">
    ...
    <xsl:template name="handle-namespace">
   	...
   	<!-- The following code workds for XSLT2 -->
            ...
            <xsl:when use-when="not(element-available('xsl:namespace'))" test="function-available('exsl:node-set')">
                ...
                <xsl:copy-of select="exsl:node-set($ns-dummy-elements)                                   /*/namespace::*[local-name()=$p]"/>
            </xsl:when>
