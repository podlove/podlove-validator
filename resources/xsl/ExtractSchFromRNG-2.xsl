...
-->
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:rng="http://relaxng.org/ns/structure/1.0" version="1.0">
    ...
    <xsl:template match="/rng:grammar | /rng:element">
        ...
        <xsl:call-template name="gatherSchema">
            ...
            <xsl:with-param name="includes" select="document(/rng:grammar/rng:include/@href | //rng:externalRef/@href)"/>
        </xsl:call-template>
    </xsl:template>
	<!-- -->
        ...
        <xsl:choose>
            <xsl:when test="count($schemas) &lt; count($schemas | $includes)">
                ...
                <xsl:call-template name="gatherSchema">
                    ...
                    <xsl:with-param name="includes" select="document($includes/rng:grammar/rng:include/@href | $includes//rng:externalRef/@href)"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
