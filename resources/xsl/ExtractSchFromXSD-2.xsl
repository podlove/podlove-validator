...
-->
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="1.0">
    ...
    <xsl:template match="xs:schema">
        ...
        <xsl:call-template name="gatherSchema">
            ...
            <xsl:with-param name="includes" select="document(/xs:schema/xs:*[self::xs:include or self::xs:import or self::xs:redefine]/@schemaLocation)"/>
        </xsl:call-template>
    </xsl:template>
        <!-- -->
        ...
        <xsl:choose>
            <xsl:when test="count($schemas) &lt; count($schemas | $includes)">
                ...
                <xsl:call-template name="gatherSchema">
                    ...
                    <xsl:with-param name="includes" select="document($includes/xs:schema/xs:*[self::xs:include or self::xs:import or self::xs:redefine]/@schemaLocation)"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template name="output">
        ...
        <sch:schema queryBinding="xslt2">
            ...
            <xsl:for-each select="$schemas//xs:appinfo/sch:ns">
                <xsl:if test="generate-id(.) =          generate-id($schemas//xs:appinfo/sch:ns[@prefix = current()/@prefix][1])">
                    <xsl:copy-of select="."/>
                </xsl:if>
            </xsl:for-each>
