...
<xsl:stylesheet xmlns:nvdl="http://purl.oclc.org/dsdl/nvdl" xmlns:iso="http://purl.oclc.org/dsdl/schematron" xmlns:relax="http://relaxng.org/ns/structure/1.0" xmlns:schold="http://www.ascc.net/xml/schematron" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:dsdl="http://www.schematron.com/namespace/dsdl" xmlns:crdl="http://purl.oclc.org/dsdl/crepdl/ns/structure/1.0" xmlns:xslt="http://www.w3.org/1999/XSL/Transform" xmlns:sch-check="http://www.schematron.com/namespace/sch-check" xmlns:dtll="http://www.jenitennison.com/datatypes" xmlns:xlink="http://www.w3.org/1999/xlink" version="3.0">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:choose>
					...
					<!-- this case is when there is in embedded schema in the same document elsewhere -->
                    <xsl:when test="string-length( $document-uri ) = 0">
                        <xsl:apply-templates mode="dsdl:go" select="//iso:*[@xml:id= $fragment-id ]                  |id( $fragment-id)                 | //iso:*[@id= $fragment-id ]"/>
                    </xsl:when>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:choose>
					...
					<!-- this case is when there is in embedded schema in the same document elsewhere -->
                    <xsl:when test="string-length( $document-uri ) = 0">
                        <xsl:apply-templates mode="dsdl:go" select="//schold:*[@xml:id= $fragment-id ]                  |id( $fragment-id)                 | //schold:*[@id= $fragment-id ]"/>
                    </xsl:when>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    ...
                    <xsl:when test="string-length( @href ) = 0">
                        <xsl:apply-templates mode="dsdl:go" select="//*[@xml:id= current()/@xpointer  ] | id( @xpointer)                | //*[@id= current()/@xpointer  ]"/>
                    </xsl:when>
	...
	<!-- =========================================================== -->
    <xsl:template match="*[@xlink:href][not(parent::*[@xlink:type='complex'])]             [not(@xlink:type) or (@xlink:type='simple')]             [@xlink:show='embed']             [not(@xlink:actuate) or (@xlink:actuate='onLoad')]" mode="dsdl:go" priority="1">
